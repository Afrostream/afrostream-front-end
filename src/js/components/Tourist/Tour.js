...
class Tour {
  ...
  reset () {
    ...
    ReactDOM.unmountComponentAtNode(this._tourDiv);
  ...
  _getStepMountNode (step) {
    try {
      const getElement = step.getElement || (x => {
          return ReactDOM.findDOMNode(x);
        });
      const element = getElement(this._tourables[step.component].refs[step.ref]);
      return element;
    } catch (err) {
      return null;
    }
  ...
  _mountTourItem (step) {
    ...
    if (typeof mountContent === 'string') {
      ...
      mountContent = ReactDOM.createElement(mountContent, {
        onDone: this.next.bind(this),
        onSkip: this.skip.bind(this)
      });
    }
    ...
    //  Adjust position
    ReactDOM.render(tourItem, this._tourDiv, () => {
  ...
  _mountIndicator (step) {
    ...
    mountNode._tourIndicator.style.position = 'fixed';
    ...
    ReactDOM.render(tourIndicator, mountNode._tourIndicator, () => {
  ...
  _unmountIndicator () {
    ...
    if (step && this._tourables[step.component] && this._tourables[step.component].refs[step.ref]) {
      const getElementFunc = step.getElement || (x => {
          return x.getDOMNode();
        });
      try {
        ReactDOM.unmountComponentAtNode(getElementFunc(this._tourables[step.component].refs[step.ref])._tourIndicator);
      } catch (err) {
        return;
      }
    }
