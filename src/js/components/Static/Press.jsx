...
class Press extends React.Component {
  ...
  renderPress () {
    ...
    let groupedPress = _.groupBy(press, (item) => {
      return item.date && item.date.substring(6)
    })

    return _.map(groupedPress, (group, groupKey)=> {
      const date = group[0].date
      return <div className="row" key={`static-press-group-${groupKey}`} id={`news-${date.substring(6)}`}>
        <h3>{date}</h3>
        <div className="items">
          {
            _.map(group, (item, key)=> {
