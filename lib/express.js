...
const testHttps = function (req, res, next) {
  if ('production' === env || 'staging' === env) {
    // enable ssl redirect
    var schema = (req.get('x-forwarded-proto') || '').toLowerCase(),
      hasW = ~req.hostname.indexOf('www');
    console.log(schema, hasW, req.hostname, req.originalUrl);
    if (schema === 'https' && !hasW) {
      next();
    } else {
      res.redirect('https://' + config.server.cname + req.originalUrl);
    }
  }
}
...
app.get('/testhttps', testHttps, function (req, res) {
  var schema = (req.get('x-forwarded-proto') || '').toLowerCase(),
    hasW = ~req.hostname.indexOf('www');
  res.json({
    url: req.url,
    ...
    redirectUrl: 'https://' + config.server.cname + req.url,
