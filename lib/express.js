...
const hash = process.env.HASH_FILE || 'none';
// Serve static files
// --------------------------------------------------
import fs from 'fs';
...
const hashValue = fs.readFileSync(path.join(buildPath, 'hash.txt'));
...
app.get('/*', (req, res) => {
  ...
  const jsPaths = ['vendor', 'main'].map(basename => {
    if (env === 'development') {
      ...
      return `//${host}:${port}/static/${basename}.${hashValue}.js`;
    }
  ...
  const cssPaths = ['main'].map(basename => {
    if (env === 'development') {
      let { webpackDevServer: { host, port } } = config;
      return `//${host}:${port}/static/${basename}.${hashValue}.css`;
    }
